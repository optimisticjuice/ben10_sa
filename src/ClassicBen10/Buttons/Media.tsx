  ...
  useEffect(() => {
    const loadMediaWithImages = async () => {
      try {
        ...
        // Only show media from Ben 10 Original series
        const originalSeriesMedia = allMedia.filter(item => 
          item.series.includes("Ben 10")
        );
        
        setMedia(originalSeriesMedia);
      } catch (err) {

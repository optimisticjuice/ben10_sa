...
const Media: React.FC = () => {
  ...
  useEffect(() => {
    const loadMediaWithImages = async () => {
      try {
        ...
        // Only show media from Ben 10 Omniverse series
        const omniverseSeriesMedia = allMedia.filter(item => 
          item.series.includes("Ben 10 Omniverse")
        );
        
        setMedia(omniverseSeriesMedia);
      } catch (err) {
  ...
  const filteredMedia = media.filter((item) => {
    ...
    if (filterType === "movies") {
      return matchesSearch && item.name.includes("Omniverse");
    }
    if (filterType === "games") {
      return matchesSearch && (item.name.includes("Omniverse") || item.name.includes("Omniverse 2"));
    }

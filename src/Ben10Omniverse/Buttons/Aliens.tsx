  ...
  useEffect(() => {
    const loadAliensWithImages = async () => {
      try {
        ...
        // Only show aliens from Ben 10 Omniverse series
        const omniverseSeriesAliens = allAliens.filter(alien => 
          alien.series.includes("Ben 10: Omniverse")
        );
        
        setAliens(omniverseSeriesAliens);
      } catch (err) {

...
const Media: React.FC = () => {
  ...
  useEffect(() => {
    const loadMediaWithImages = async () => {
      try {
        ...
        // Only show media from Ben 10 Alien Force series
        const alienForceSeriesMedia = allMedia.filter(item => 
          item.series.includes("Ben 10 Alien Force")
        );
        
        setMedia(alienForceSeriesMedia);
      } catch (err) {
  ...
  const filteredMedia = media.filter((item) => {
    ...
    if (filterType === "movies") {
      return matchesSearch && item.name.includes("Alien Swarm");
    }
    if (filterType === "games") {

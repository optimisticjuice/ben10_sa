  ...
  useEffect(() => {
    const loadAliensWithImages = async () => {
      try {
        ...
        // Only show aliens from Ben 10 Alien Force series
        const alienForceSeriesAliens = allAliens.filter(alien => 
          alien.series.includes("Ben 10: Alien Force")
        );
        
        setAliens(alienForceSeriesAliens);
      } catch (err) {

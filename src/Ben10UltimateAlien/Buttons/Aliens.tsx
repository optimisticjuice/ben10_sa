  ...
  useEffect(() => {
    const loadAliensWithImages = async () => {
      try {
        ...
        // Only show aliens from Ben 10 Ultimate Alien series
        const ultimateAlienSeriesAliens = allAliens.filter(alien => 
          alien.series.includes("Ben 10: Ultimate Alien")   
        );
        
        setAliens(ultimateAlienSeriesAliens);
      } catch (err) {

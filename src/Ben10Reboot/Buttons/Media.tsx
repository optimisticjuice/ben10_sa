...
const Media: React.FC = () => {
  ...
  useEffect(() => {
    const loadMediaWithImages = async () => {
      try {
        ...
        // Only show media from Ben 10 Reboot series
        const RebootSeriesMedia = allMedia.filter(item => 
          item.series.includes("Ben 10 Reboot")
        );
        
        setMedia(RebootSeriesMedia);
      } catch (err) {
  ...
  const filteredMedia = media.filter((item) => {
    ...
    if (filterType === "movies") {
      return matchesSearch && !item.name.includes("Ben 10 Reboot") && !item.name.includes("Protector") && !item.name.includes("Vilgax") && !item.name.includes("Alien Force") && !item.name.includes("Ultimate Alien") && !item.name.includes("Omniverse") && !item.name.includes("Galactic") && !item.name.includes("PunchTime") && !item.name.includes("Mind Mine") && !item.name.includes("(2017)") && !item.name.includes("Power Trip");
    }
    if (filterType === "games") {
      return matchesSearch && (item.name.includes("Ben 10 Reboot") || item.name.includes("Protector") || item.name.includes("Vilgax") || item.name.includes("Alien Force") || item.name.includes("Ultimate Alien") || item.name.includes("Omniverse") || item.name.includes("Galactic") || item.name.includes("PunchTime") || item.name.includes("Mind Mine") || item.name.includes("(2017)") || item.name.includes("Power Trip"));
    }

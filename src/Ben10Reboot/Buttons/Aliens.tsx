  ...
  useEffect(() => {
    const loadAliensWithImages = async () => {
      try {
        ...
        // Only show aliens from Ben 10 Reboot series
        const rebootSeriesAliens = allAliens.filter(alien => 
          alien.series.includes("Ben 10 Reboot")
        );
        
        setAliens(rebootSeriesAliens);
      } catch (err) {
